...
export default function QuizStepPage() {
  ...
  for (let i = 0; i < quizBlocks.length; i++) {
    ...
    if (step > cumulativeQuestions && step <= cumulativeQuestions + block.questions.length) {
      currentBlockIndex = i
      currentQuestionIndex = step - cumulativeQuestions - 1
      break
    }
    cumulativeQuestions += block.questions.length
  }

  if (currentBlockIndex === -1 && step !== totalQuestions + 1) {
    // Invalid step, redirect to start of quiz
    useEffect(() => {
      router.push("/quiz/1")
    }, [router])
    return null
  ...
  const handleAnswer = (questionId: string, answer: string) => {
    ...
    const currentBlockData = quizBlocks[currentBlockIndex]
  ...
  const createStripeSession = async () => {
    ...
    try {
      ...
      if (data.url) {
        ...
        alert(
          "This is a development preview. In a production environment, you would be redirected to Stripe to complete the payment."
        )
      } else {
  ...
  const currentBlockData = quizBlocks[currentBlockIndex]
  const currentQuestionData = currentBlockData.questions[currentQuestionIndex]
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-blue-50">
      ...
      <div className="bg-white border-b border-gray-200 p-4">
        <div className="max-w-4xl mx-auto">
          <Progress value={(step / totalQuestions) * 100} className="h-2" />
          <div className="flex justify-between mt-2 text-xs text-gray-500">
            <span>Progress: {Math.round((step / totalQuestions) * 100)}%</span>
            <span>
        ...
        <Card className="max-w-2xl w-full">
          <CardContent className="p-8">
            ...
            <div className="text-center mb-8">
              ...
              >
                ...
                <span className="font-semibold">
                  Block {currentBlockIndex + 1}: {currentBlockData.title}
                </span>
              </div>
            ...
            <div className="flex justify-between items-center">
              ...
              >
                <span>{step === totalQuestions ? "Finish" : "Next"}</span>
